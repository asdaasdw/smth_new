...
LRESULT CALLBACK GraphProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_PAINT:
    {
        ...
        if (T_sum > tMin & T_sum <= tMax_1)
        {
            ...
            // grid
            HPEN hPen0=CreatePen(PS_SOLID, 1, RGB(200, 230, 200));
            HPEN hOldPen=(HPEN)SelectObject(hdc, hPen0);
        ...
        else if (T_sum > tMax_2 & T_sum <= tMax_5)
        {
            const double tGridStep_5_sec = 1;
            int xMin = tMin * nPixPerT;
            int xMax_5 = tMax_1 * nPixPerT;
            ...
            for (int j=1; j<(sizeof(xMark_5)/sizeof(xMark_5[0])); ++j) // horizontal
            {
                x = horizontal_grid_0 * nPixPerT;
                MoveToEx(hdc, x + 20, yMin, NULL);
                LineTo(hdc, x + 20, yMax);
                horizontal_grid_0 += tGridStep_5_sec;
                TextOutA(hdc, x+40, 3, xMark_5[j], strlen(xMark_5[j]));
            }

            // plot
            double vertical_step_grid = (yMax - yMin) / 60;
            double horizontal_step_grid = (xMax_5 - xMin) / (60 * 5);
        ...
        else if (T_sum > tMax_5 & T_sum <= tMax_10)
        {
            const double tGridStep_10_sec = 1;
            int xMin = tMin * nPixPerT;
            int xMax_10 = tMax_1 * nPixPerT;
            ...
            for (int j=1; j<(sizeof(xMark_10)/sizeof(xMark_10[0])); ++j) // horizontal
            {
                x = horizontal_grid_0 * nPixPerT;
                MoveToEx(hdc, x + 20, yMin, NULL);
                LineTo(hdc, x + 20, yMax);
                horizontal_grid_0 += tGridStep_10_sec;
                TextOutA(hdc, x+40, 3, xMark_10[j], strlen(xMark_10[j]));
            }

            // plot
            double vertical_step_grid = (yMax - yMin) / 60;
            double horizontal_step_grid = (xMax_10 - xMin) / (60 * 10);
        ...
        else if (T_sum > tMax_10 & T_sum <= tMax_30)
        {
            const double tGridStep_30_sec = 1;
            int xMin = tMin * nPixPerT;
            int xMax_30 = tMax_1 * nPixPerT;
            ...
            for (int j=1; j<(sizeof(xMark_30)/sizeof(xMark_30[0])); ++j) // horizontal
            {
                x = horizontal_grid_0 * nPixPerT;
                MoveToEx(hdc, x + 20, yMin, NULL);
                LineTo(hdc, x + 20, yMax);
                horizontal_grid_0 += tGridStep_30_sec;
                TextOutA(hdc, x+40, 3, xMark_30[j], strlen(xMark_30[j]));
            }

            // plot
            double vertical_step_grid = (yMax - yMin) / 60;
            double horizontal_step_grid = (xMax_30 - xMin) / (60 / 2);
            ...
            {
                double nextY = - Omega * vertical_step_grid;
                LineTo(hdc, 20 + (T_new / 60) * horizontal_step_grid, currentY);
        ...
        else if (T_sum > tMax_30)
        {
            const double tGridStep_1_min = 1;
            int xMin = tMin * nPixPerT;
            int xMax_max = tMax_1 * nPixPerT;
            ...
            // plot
            double vertical_step_grid = (yMax - yMin) / 60;
            double horizontal_step_grid = (xMax_max - xMin) / (60);
